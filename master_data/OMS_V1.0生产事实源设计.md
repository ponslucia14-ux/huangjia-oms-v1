...
该定义适用于V1.0初始化迁移和生产基线冻结。正式运行切换完成后，OMS Domain Current成为日常经营第一事实源；`OMS_TRUTH_SOURCE`中的激活快照作为初始化基线、追溯证据和外部导入暂存边界，不再由员工长期通过Excel直接维护。

完整切换规则见《OMS事实源迁移与运行切换设计.md》。
